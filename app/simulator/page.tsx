...
function SimulationResults({
	...
	// Helper function to reshape 1D field data into 2D array for heatmap
	const reshapeFieldData = (
		data: number[],
		x?: number[],
		y?: number[]
	): number[][] => {
		if (x && y) {
			...
			const result: number[][] = Array(uniqueY.length)
				.fill(0)
				.map(() => Array(uniqueX.length).fill(0));

			for (let i = 0; i < data.length; i++) {
				const xIdx = uniqueX.indexOf(x[i]);
				const yIdx = uniqueY.indexOf(y[i]);
				result[yIdx][xIdx] = data[i];
			}
			return result;
		}
