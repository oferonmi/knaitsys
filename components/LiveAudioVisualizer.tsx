...
import { calculateBarData, draw } from "@/components/utils";
...
const LiveAudioVisualizer: (props: Props) => ReactElement = ({
    ...
    barColor = "#D9E2D5", //"rgb(160, 198, 255)",
    fftSize = 1024,
    maxDecibels = -10,
    minDecibels = -90,
    ...
    }, [context, fftSize, maxDecibels, mediaRecorder.stream, minDecibels, smoothingTimeConstant]);
    ...
    }, [analyser, context, mediaRecorder.state, processFrequencyData]);

	useEffect(() => {
        if (analyser && mediaRecorder.state === "recording") {
        	report();
        }
    }, [analyser, mediaRecorder.state, report]);
